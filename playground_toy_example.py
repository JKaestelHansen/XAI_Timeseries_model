...
model = Transformer(seq_len=50).to(device)
...
for epoch in trange(100, desc="Training"):
    model.train()
    optimizer.zero_grad()
    output = model(X_train)  # [B, T, 2]
    loss = criterion(output.permute(0, 2, 1), y_train)  # [B, 2, T], [B, T]
    loss.backward()
    optimizer.step()
    ...
    with torch.no_grad():
        ...
        val_preds = torch.sigmoid(val_output)[:,:,1] > 0.5
        val_preds = val_preds.cpu().numpy().flatten()

        val_targets = y_val.cpu().numpy().flatten()

        acc = accuracy_score(val_targets, val_preds)
        f1 = f1_score(val_targets, val_preds, average="binary")
        
        val_loss = criterion(val_output.permute(0, 2, 1), y_val).item()
        ...
        TP_history.append(TP.item())
        TN_history.append(TN.item())
        FP_history.append(FP.item())
        FN_history.append(FN.item())
...
model.eval()
input_ts = X_val[0:1].clone().detach().requires_grad_()
pred = model(input_ts)
...
ax[1].plot(probs.squeeze().detach().cpu(), label='Predicted Probabilities')
